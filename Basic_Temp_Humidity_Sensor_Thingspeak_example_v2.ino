...
#define channelKey "WRITE_API_KEY"
...
void loop() {
  if (espPresent){
    ...
    while (!espAsleep){
      Serial1.println(F("AT+GSLP=900000")); //put the ESP8266 to sleep
      if (recFind("OK", 5000)){
...
void initEsp(){  bool espReady = false;
...
struct reading readSensor(){
  ...
  Serial.print(reading_r.f);
  Serial.println("F");
  Serial.print("Humidity: ");
  Serial.print(reading_r.h);
  Serial.println("%");
  return reading_r;
}

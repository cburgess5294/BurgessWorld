...
void setup() {
  ...
  Wire1.onReceive(receiveEvent); // register event when data received
  Wire1.onRequest(sendData);     // wait for request from master before sending Ack
...
void receiveEvent(int howMany) {
  while (Wire1.available()) { // loop through all the received data
...
void sendData(){
    ...
    Wire1.write(0x01);  //send a 1 to master acknowlege the data
}
